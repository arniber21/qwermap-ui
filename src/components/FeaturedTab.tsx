...
export default function FeaturedTab({
	...
	if (hideWhenSelected && selectedPlace !== null) return null;
	if (topPlaces.length === 0) return null;

	const handleClick = async (placeId: string, coords: [number, number]) => {
		...
		<>
			...
			>
				<div className="flex items-center gap-2">
					<Star size={14} className="text-mauve" fill="currentColor" />
	...
	const Content =
		variant === 'card' ? (
			<div
				className={cn(
					'bg-surface-elevated/90 backdrop-blur-md rounded-3xl shadow-lg border border-border overflow-hidden',
					className
				)}
			>
				{Inner}
	...
	return <div className="fixed top-20 right-6 lg:right-[30rem] z-20 w-72">{Content}</div>;
}
